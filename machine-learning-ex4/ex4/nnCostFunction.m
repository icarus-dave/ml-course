...
a_1_v = [ones(rows(X),1) X];
z_2_v = Theta1*a_1_v';
a_2_v = [ones(rows(z_2_v'),1) sigmoid(z_2_v)'];
z_3_v = Theta2*a_2_v';
a_3_v = sigmoid(z_3_v);
...
Theta1_grad = Theta1_grad + [zeros(rows(Theta1),1) RegTheta1(:,2:end)];
Theta2_grad = Theta2_grad + [zeros(rows(Theta2),1) RegTheta2(:,2:end)];
